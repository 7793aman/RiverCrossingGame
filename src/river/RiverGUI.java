...
public class RiverGUI extends JPanel implements MouseListener {
    ...
    private Item[] objectsOnBoat = new Item[2];
    public void loadObjectsOnBoat(Item item){
        engine.loadBoat(item);
        if(objectsOnBoat[0] == null) {
            objectsOnBoat[0] = item;
        }
        else if(objectsOnBoat[1]  == null){
            objectsOnBoat[1] = item;
        }
    }
    public void unloadObjectsOnBoat(Item item, Map<Item, Rectangle> map){
        engine.unloadBoat(item);
        if(objectsOnBoat[0] == item){
            objectsOnBoat[0] = null;

        }
        else if((objectsOnBoat[1] == item)){
            objectsOnBoat[1] = null;
        }
        map.remove(item);
    }
    public Rectangle getItemOnBoatRect(Item item, Location location){
        ...
        if(location == Location.START){
             rect = new Rectangle(boatLeftBase + dX, 215, 50, 50);
            leftBoatRectMap.put(item,rect);
            return rect;
        }
        rect=new Rectangle(boatRightBase + dX, 215, 50, 50);
        rightBoatRectMap.put(item,rect);
        return rect;
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        for (Item item : Item.values()) {
            ...
            if(leftBoatRectMap.get(item)!=null && leftBoatRectMap.get(item).contains(e.getPoint()) && engine.getItemLocation(item) == Location.BOAT){
                unloadObjectsOnBoat(item,leftBoatRectMap);
            }
            if(rightBoatRectMap.get(item)!=null && rightBoatRectMap.get(item).contains(e.getPoint()) && engine.getItemLocation(item) == Location.BOAT){
                unloadObjectsOnBoat(item,rightBoatRectMap);
            }
        }
