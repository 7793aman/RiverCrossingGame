...
public class MonsterGameEngine extends AbstractGameEngine {
    ...
    public MonsterGameEngine() {
        setGameObjects(new HashMap<Item, GameObject>() {{
            put(MUNCHKIN_1, new GameObject("H", Location.START, Color.CYAN));
            put(MUNCHKIN_2, new GameObject("H", Location.START, Color.CYAN));
            put(MUNCHKIN_3, new GameObject("H", Location.START, Color.CYAN));
            put(MONSTER_1, new GameObject("M", Location.START, Color.PINK));
            put(MONSTER_2, new GameObject("M", Location.START, Color.PINK));
            put(MONSTER_3, new GameObject("M", Location.START, Color.PINK));
        }});
    ...
    public boolean gameIsLost() {
        ...
        for (Item item : Item.values()) {
            ...
            if (item.ordinal() % 2 == 0) {
                if (gamePlayers.get(item).getLocation() == Location.START ||
                        (gamePlayers.get(item).getLocation() == Location.BOAT && getBoatLocation() == Location.START))
                    leftMonsters++;
                else rightMonsters++;
            } else {
                if (gamePlayers.get(item).getLocation() == Location.START ||
